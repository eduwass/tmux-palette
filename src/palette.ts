...
export function buildNavKeys(vimKeys: boolean): Record<string, number> {
  return vimKeys
    ? { ...NAV_KEYS, "\x0b": -1, "\x0a": 1, "\x15": -10, "\x04": 10 }
    : { ...NAV_KEYS };
}
...
export async function runPalette(
  ...
  function render(): void {
    ...
    const sel =
      selectionAnchor !== null && selectionAnchor !== filterCursor
        ? {
            start: Math.min(selectionAnchor, filterCursor),
            end: Math.max(selectionAnchor, filterCursor),
          }
        : undefined;
    const inner = [
      header.line,
  ...
  async function dispatchApplyAction(
    fn: (ctx: ActionContext) => void | Promise<void>,
  ): Promise<void> {
    await fn({ cmdFile });
  ...
  function handleEditKey(key: string): boolean {
    ...
    if (key === "\x1b[1;2F") {
      extendTo(filter.length);
      return true;
    }
    if (key === "\x1b[1;4D" || key === "\x1b[1;6D") {
      extendTo(wordBack(filter, filterCursor));
      return true;
    }
    if (key === "\x1b[1;4C" || key === "\x1b[1;6C") {
      extendTo(wordForward(filter, filterCursor));
      return true;
    }
