...
function descriptionFragment(
  item: Item,
  colors: Colors,
  rowBg: string,
): { styled: string; width: number } {
...
function shortcutFragment(
  item: Item,
  colors: Colors,
  active: boolean,
  rowBg: string,
): { styled: string; text: string } {
...
export function composeListBody(
  ...
  for (let i = scroll; i < end; i++) {
    ...
    if (row.kind === "item")
      rowActions.push({ y: startY + (i - scroll), itemIndex: row.itemIndex });
