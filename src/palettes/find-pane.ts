...
function groupPanes(panes: Pane[]): {
  sessionOrder: string[];
  bySession: Map<string, Map<string, WindowGroup>>;
} {
...
function windowItem(
  ...
  return {
    title: w.windowName,
    action: {
      tmux: `select-window -t ${tmuxQuote(`${session}:${windowIndex}`)} \\; switch-client -t ${tmuxQuote(session)}`,
    },
    selectable: false,
    data: {
      kind: "window",
...
function renderSession(
  data: Extract<ItemData, { kind: "session" }>,
  colors: Colors,
  rowBg: string,
): string {
...
function renderPane(
  ...
  const titleStyle = active ? colors.bold + colors.fg : p.isCurrent ? colors.fg : colors.muted;
...
function filterTree(items: Item[], query: string): Item[] {
  ...
  for (const item of items) {
    ...
    const p = data.pane;
    const haystack = [p.session, p.windowName, p.paneTitle, p.command, p.path, p.target, p.agent]
      .filter(Boolean)
      .join(" ");
